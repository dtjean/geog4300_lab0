...
{Ordinal: MONTH_NAME this variable is ordinal because months have an
order but are not measurements

Ratio: TOR_WIDTH this variable is a ratio because it has a true zero and
widths scale relatively and linearly (ex: a width of 10 feet is half as
big as a 20 foot width)}
...
F3orMore <- tornadoes %>%
  ...
  summarise(number_of_tornadoes = n())
```
...
``` r
svi_social_vulnerability <- svi25000orMore %>%
  filter(F_DISABL == 1)
```
...
``` r
svi_social_vulnerability <- svi_social_vulnerability %>%
  rename(cty_fips = FIPS)
  combined_data <- inner_join(svi_social_vulnerability, F3orMore, by = "cty_fips")
```
...
{inner_join() combines two datasets to a matching variable. This is done
by keeping only that which matches between both datasets. The results
would have been different if I had used full_join() because every row
would be in it, all variables between both datasets would still be
present.}
...
{\`\`\`{R} names(combined_data)

combined_data %\>% arrange(desc(number_of_tornadoes)) %\>%
select(STATE.x, COUNTY, number_of_tornadoes) %\>% head(3) \`\`\` Walker
County, Alabama (8 severe tornadoes); Sequoyah County, Oklahoma (8
severe tornadoes); Marion County, Alabama (5 severe tornadoes)

Walker County has Jasper, Alabama and a National Forest. Sequoyah County
has Sallisaw, Oklahoma, and the Illinois River. Marion County has four
cities: Hamilton, Guin, Winfield, and Hackleberg. }
...
I used ChatGPT and Google searches to assist in my answering of these
tasks. They helped by explaining how I could do this work myself,
through instructional content framed as a worksheet of sorts for this
worksheet.
...
It was easy, but I had several roadblocks in regards to illnesses and
family medical issues which caused this to be later than I would have
liked. This should not be a problem going forward. I feel as though I am
now much more familiar with the logic and construction of R, at least
for the commands and functions which this assignment covered.
